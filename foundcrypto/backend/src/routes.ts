...
import { createCryptoRouter } from './crypto.js'
...
const requireFoundCryptoModule = createModuleAccessMiddleware('foundcrypto')
...
apiRouter.get('/status', (_req, res) => res.json({ app: 'foundcrypto', status: 'operational' }))
...
apiRouter.get('/whatsapp/status', requireOwnerAccess, requireTenant, requireFoundCryptoModule, (_req, res) => res.json({ success: true, data: whatsappReadiness() }))
apiRouter.post('/whatsapp/messages', requireMerchantAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
  try { res.status(202).json({ success: true, data: await sendWhatsAppText(req.body?.to, req.body?.text) }) } catch (error) { next(error) }
})
apiRouter.use('/crypto', requireMerchantAccess, requireTenant, requireFoundCryptoModule, createCryptoRouter())
apiRouter.use('/bob', requireMerchantAccess, requireTenant, requireFoundCryptoModule, createBobRouter('foundcrypto'))
apiRouter.get('/console/products', requireMerchantAccess, requireTenant, requireFoundCryptoModule, async (_req, res, next) => { try { const data = await merchantWorkspace(res.locals.auth.tenantId, res.locals.auth.id); res.json({ products: data.inventory }) } catch (error) { next(error) } })
apiRouter.get('/console/orders', requireMerchantAccess, requireTenant, requireFoundCryptoModule, async (_req, res, next) => { try { const data = await merchantWorkspace(res.locals.auth.tenantId, res.locals.auth.id); res.json({ orders: data.orders }) } catch (error) { next(error) } })
apiRouter.get('/console/customers', requireMerchantAccess, requireTenant, requireFoundCryptoModule, async (_req, res, next) => { try { const data = await pipelineSummary(res.locals.auth.tenantId); res.json({ customers: data.customers }) } catch (error) { next(error) } })
apiRouter.get('/console/reports', requireMerchantAccess, requireTenant, requireFoundCryptoModule, async (_req, res, next) => { try { const [pipeline, operations] = await Promise.all([pipelineSummary(res.locals.auth.tenantId), operationsSummary(res.locals.auth.tenantId)]); res.json({ reports: { pipeline: pipeline.metrics, operations: operations.metrics } }) } catch (error) { next(error) } })
apiRouter.get('/merchant/workspace', requireMerchantAccess, requireTenant, requireFoundCryptoModule, async (_req, res, next) => {
...
apiRouter.post('/merchant/changes', requireMerchantAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
...
apiRouter.get('/merchants/:id', requireMerchantAccess, requireTenant, requireFoundCryptoModule, (req, res) => res.json({ merchant: { id: req.params.id, tenantId: res.locals.auth.tenantId } }))
apiRouter.get('/consoles/:id', requireMerchantAccess, requireTenant, requireFoundCryptoModule, (req, res) => res.json({ console: { id: req.params.id, tenantId: res.locals.auth.tenantId } }))
apiRouter.get('/packages/:id', requireOwnerAccess, requireTenant, requireFoundCryptoModule, (req, res) => res.json({ package: { id: req.params.id, tenantId: res.locals.auth.tenantId } }))
apiRouter.get('/owner/overview', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (_req, res, next) => {
...
apiRouter.get('/owner/pipeline', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (_req, res, next) => {
...
apiRouter.get('/customers', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
  try { res.json({ success: true, data: await listCustomers(readTenant(req, res)) }) } catch (error) { next(error) }
})
apiRouter.get('/customers/:id', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
  try { res.json({ success: true, data: await getCustomer(String(req.params.id), readTenant(req, res)) }) } catch (error) { next(error) }
})
apiRouter.post('/customers', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
...
apiRouter.patch('/customers/:id', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
...
apiRouter.delete('/customers/:id', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
  try { res.json({ success: true, data: await deleteCustomer(String(req.params.id), readTenant(req, res)) }) } catch (error) { next(error) }
})
apiRouter.post('/leads', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
...
apiRouter.patch('/leads/:id', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
...
apiRouter.post('/leads/:id/convert', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
...
apiRouter.get('/owner/operations', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
  try { res.json({ success: true, data: await operationsSummary(readTenant(req, res)) }) } catch (error) { next(error) }
})
apiRouter.get('/owner/merchants', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
...
apiRouter.post('/owner/merchants/staff', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req,res,next)=>{
...
apiRouter.patch('/owner/merchants/staff/:id', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req,res,next)=>{
...
apiRouter.delete('/owner/merchants/staff/:id', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req,res,next)=>{
...
apiRouter.post('/owner/merchants/staff/:id/reset-password', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req,res,next)=>{
...
apiRouter.patch('/owner/merchants/changes/:id', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req,res,next)=>{
...
apiRouter.get('/inventory', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
  try { res.json({ success: true, data: await searchInventory(readTenant(req, res), req.query) }) } catch (error) { next(error) }
})
apiRouter.post('/inventory', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
...
apiRouter.patch('/inventory/:id', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
...
apiRouter.delete('/inventory/:id', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
  try { res.json({ success: true, data: await deleteInventoryItem(String(req.params.id), readTenant(req, res)) }) } catch (error) { next(error) }
})
apiRouter.post('/orders', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
...
apiRouter.patch('/orders/:id', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
  try { res.json({ success: true, data: await updateOrder(String(req.params.id), readTenant(req, res), req.body || {}) }) } catch (error) { next(error) }
})
apiRouter.post('/invoices', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
...
apiRouter.patch('/invoices/:id', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
...
apiRouter.post('/invoices/:id/send', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
  try { res.json({ success: true, data: await sendInvoice(String(req.params.id), readTenant(req, res)) }) } catch (error) { next(error) }
})
apiRouter.get('/invoices/:id/download', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
...
apiRouter.post('/marketing/campaigns', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
...
apiRouter.patch('/marketing/campaigns/:id', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
...
apiRouter.post('/social/posts', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
...
apiRouter.patch('/social/posts/:id', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
...
apiRouter.post('/media/generate', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
...
apiRouter.post('/delivery/operators', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
...
apiRouter.patch('/delivery/operators/:id', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
...
apiRouter.post('/delivery/vehicles', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
...
apiRouter.patch('/delivery/vehicles/:id', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
...
apiRouter.post('/delivery/zones', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
...
apiRouter.patch('/delivery/zones/:id', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
...
apiRouter.post('/delivery/assignments', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
...
apiRouter.patch('/delivery/assignments/:id', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
...
apiRouter.patch('/delivery/notifications/:id', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
...
apiRouter.post('/delivery/notifications/:id/send', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
...
apiRouter.post('/location/detect', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
  try { res.json({ success: true, data: await detectLocation(req.body || {}, req.ip) }) } catch (error) { next(error) }
})
apiRouter.put('/location/profile', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
...
apiRouter.get('/location/weather', requireOwnerAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
...
apiRouter.get('/owner/staff', requireOwnerAccess, requireTenant, requireFoundCryptoModule, (_req, res) => res.json({ staff: [] }))
apiRouter.get('/owner/settings', requireOwnerAccess, requireTenant, requireFoundCryptoModule, (_req, res) => res.json({ settings: {} }))
apiRouter.post('/media', requireMerchantAccess, requireTenant, requireFoundCryptoModule, async (req, res, next) => {
