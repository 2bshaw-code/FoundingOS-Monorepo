...
import { BobMediaStudio, Card, DeliverySuite, EntityAnalytics, ImageBlock, InsightPanel, InventoryModule, InvoicesModule, MarketingSuite, MerchantPerformanceOverview, OperationsAnalytics, OrdersModule, OwnerMerchantManagement, PasskeySettings, PipelineAnalytics, PremiumConsole, SocialScheduler, Table, useCompanyBrand, WeatherLocationSuite, WeatherLocationWidget, CoreOperationsLogo } from '@founder-os/ui'
...
export function CoreOperationsOwnerConsole() {
   ...
   { name: 'product-closeup.mp4', kind: 'Video', size: '5.1 MB' },
	...
	const load = async () => { try { setData((await authClient.request<{ success: true; data: Pipeline }>('/owner/pipeline')).data); setMessage('') } catch (error) { setMessage(error instanceof Error ? error.message : 'CoreOperations data unavailable') } }
  ...
  return <div className="space-y-3"><button type="button" onClick={() => { window.location.href = 'http://localhost:4000/console' }} className="inline-flex items-center gap-2 rounded-full border border-[#25D366]/30 bg-white px-4 py-2 text-sm font-semibold text-[#172126] shadow-sm">↩ Back to FoundingOS</button><PremiumConsole brand="CoreOperations" eyebrow="Company-scoped operations" title="Retail Manager Console" description="Messaging, upload flows, media review, orders, inventory, staff, and intelligent retail operations." tabs={tabs} activeTab={tab} onTabChange={setTab} accent={brandColor} logo={<CoreOperationsLogo className="h-11 w-11"/>}>
		{message && <p role="alert" className="border border-red-200 bg-red-50 p-3 text-sm text-red-700">{message}</p>}
		{tab === 'overview' && <>
     <Card title="CoreOperations dashboard preview"><ImageBlock variant="core_operations-dashboard" alt="CoreOperations console preview" caption="Retail manager control centre" glow="#25D366" /></Card>
