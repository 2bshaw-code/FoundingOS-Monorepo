...
export function IntelligenceLogo({ className = 'h-10 w-10' }: { className?: string }) { return <svg className={className} viewBox="0 0 48 48" role="img" aria-label="Intelligence"><rect width="48" height="48" rx="8" fill="#FFD600"/><path d="M14 30c0-9 7-16 20-16 0 12-7 20-18 20h-2v-4Z" fill="#FFEA70"/><path d="M15 35c5-8 11-12 19-16" fill="none" stroke="#2E2E2E" strokeWidth="3" strokeLinecap="round"/><rect x="12" y="34" width="7" height="4" rx="2" fill="#806B00"/></svg> }
// DEPRECATED: FoundMeat has been removed from the active product. Retained
// only for historical reference; not exported or imported anywhere.
function FoundMeatLogo({ className = 'h-10 w-10' }: { className?: string }) { return <svg className={className} viewBox="0 0 48 48" role="img" aria-label="FoundMeat (deprecated)"><rect width="48" height="48" rx="8" fill="#B00020"/><path d="M14 17h8v14c0 4 2 6 6 6s6-2 6-6V17h-4v13c0 2-1 3-2 3s-2-1-2-3V17h8v-5H14v5Z" fill="#FFF7F5"/><rect x="12" y="37" width="24" height="4" rx="2" fill="#EF9A9A"/></svg> }
export function CoreOperationsLogo({ className = 'h-10 w-10' }: { className?: string }) {
  return <svg className={className} viewBox="0 0 48 48" role="img" aria-label="CoreOperations"><rect width="48" height="48" rx="12" fill="#25D366"/><rect x="4" y="4" width="40" height="40" rx="10" fill="#0F172A" opacity="0.12"/><text x="13" y="31" fontSize="22" fontWeight="800" fontFamily="Inter, Segoe UI, sans-serif" fill="#FFFFFF" letterSpacing="-1.5">F</text><text x="27" y="28" fontSize="14" fontWeight="800" fontFamily="Inter, Segoe UI, sans-serif" fill="#E9FFF4" letterSpacing="-0.4">r</text></svg>
}
// DEPRECATED: FoundCrypto has been removed from the active product. Retained
// only for historical reference; not exported or imported anywhere.
function FoundCryptoLogo({ className = 'h-10 w-10' }: { className?: string }) { return <svg className={className} viewBox="0 0 48 48" role="img" aria-label="FoundCrypto (deprecated)"><rect width="48" height="48" rx="8" fill="#7C3AED"/><path d="M24 6l13 7.5v15L24 36l-13-7.5v-15L24 6Z" fill="#E9D5FF"/><path d="M24 13l6.5 3.75v7.5L24 28l-6.5-3.75v-7.5L24 13Z" fill="#7C3AED"/><path d="M24 17v14M17 24h14" stroke="#FFFFFF" strokeWidth="2.5" strokeLinecap="round"/></svg> }
// Reference the deprecated marks so they are not flagged as unused exports
// by strict linting, without exporting them for reuse.
void FoundMeatLogo
void FoundCryptoLogo
...
export type BobClient = IntelligenceAIClient

export function IntelligenceAIAssistant({ client, appName }: { client: IntelligenceAIClient; appName: string }) {
  const [prompt, setPrompt] = useState('')
  const [messages, setMessages] = useState<Array<{ from: 'user' | 'foundai'; text: string }>>([{ from: 'foundai', text: `IntelligenceAI is ready in ${appName}.` }])
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
     ...
     setMessages((current) => [...current, { from: 'foundai', text: error instanceof Error ? error.message : 'IntelligenceAI is unavailable' }])
    } finally { setPending(false) }
  }
  return <div className="mx-auto flex min-h-[560px] max-w-3xl flex-col overflow-hidden rounded-lg border border-[var(--line)] bg-white shadow-[0_8px_24px_rgba(15,23,42,0.08)]"><header className="border-b border-[var(--line)] px-5 py-4"><p className="text-sm font-semibold text-[var(--primary)]">FOUND AI</p><h1 className="text-xl font-semibold">Operations assistant</h1></header><div className="flex-1 space-y-3 overflow-y-auto bg-[var(--surface)] p-5">{messages.map((message, index) => <p key={`${message.from}-${index}`} className={`max-w-[85%] rounded-lg px-4 py-3 text-sm ${message.from === 'user' ? 'ml-auto bg-[var(--primary)] text-white' : 'border border-[var(--line)] bg-white'}`}>{message.text}</p>)}</div><form onSubmit={submit} className="flex gap-3 border-t border-[var(--line)] p-4"><label className="sr-only" htmlFor="foundai-prompt">Message IntelligenceAI</label><input id="foundai-prompt" value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder="Ask IntelligenceAI" className="min-w-0 flex-1 rounded border border-[var(--line)] px-3 py-2"/><button disabled={pending} className="rounded bg-[var(--primary)] px-4 py-2 font-semibold text-white">{pending ? 'Sending...' : 'Send'}</button></form></div>
}

export const BobAssistant = IntelligenceAIAssistant

export function IntelligenceAIFloatingButton() {
  const navigate = useNavigate()
  return <button type="button" onClick={() => navigate('/foundai')} aria-label="Open IntelligenceAI" title="Open IntelligenceAI" className="bob-ai-circle fixed bottom-5 right-5 z-40 grid h-14 w-14 place-items-center rounded-full border-2 border-white bg-[var(--primary)] text-lg font-bold text-white shadow-[0_6px_18px_rgba(15,23,42,0.22)] transition hover:-translate-y-0.5 active:translate-y-0">AI</button>
}

export const BobFloatingButton = IntelligenceAIFloatingButton
...
export function LoginForm({ title, onLogin, onPasskeyLogin }: { title: string; onLogin: (email: string, password: string) => Promise<void>; onPasskeyLogin?: (email: string) => Promise<void> }) {
  ...
  const [pending, setPending] = useState(false)
  // NOTE: this file is unused by any app (superseded by packages/ui); retained
  // for historical reference only.
  const brand = title.startsWith('FoundingOS') ? 'founding-os' : title.startsWith('CoreOperations') ? 'core_operations' : title.startsWith('FoundCrypto') ? 'foundcrypto' : title.startsWith('Intelligence') ? 'foundthis' : title.startsWith('CoreWorkforce') ? 'core_workforce' : title.startsWith('FoundMeat') ? 'foundmeat' : 'founding-os'
  const submit = async (event: FormEvent) => {
    event.preventDefault()
    setPending(true)
    setMessage('')
