...
export function OperationsAnalytics({client,domain}:{client:AnalyticsClient;domain:AnalyticsDomain}){
 ...
 const content=(()=>{
  switch(domain){
   ...
   case'media':return{cards:[['Generations',String(data.media.length)],['Formats',String(new Set(data.media.map(item=>item.format)).size)],['This week',String(data.media.filter(item=>Date.now()-new Date(String(item.createdAt||0)).getTime()<604800000).length)],['Usage trend',String(recentTrend(data.media).reduce((sum,item)=>item.value,0))]],title:'IntelligenceAI usage by format',chart:<BarChart data={countBy(data.media,'format')}/>}
