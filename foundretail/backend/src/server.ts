...
app.get('/health', (_req, res) => res.json({ app: 'foundretail', status: 'ok' }))
app.use('/api/v1/auth', createRateLimit({ windowMs: 15 * 60_000, max: 20 }))
app.use('/api/v1/retail/auth', createRateLimit({ windowMs: 15 * 60_000, max: 20 }))
app.use('/api/v1', createRateLimit({ max: 240 }))
app.use('/api/v1/auth', authRouter)
app.use('/api/v1/retail/auth', authRouter)
app.use('/api/v1', apiRouter)
app.use(structuredErrorHandler)
app.listen(port, () => console.log(`FoundRetail API listening on ${port}`))
