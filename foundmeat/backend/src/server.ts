...
app.get('/health', (_req, res) => res.json({ app: 'foundmeat', status: 'ok' }))
app.use('/api/v1/auth', createRateLimit({ windowMs: 15 * 60_000, max: 20 }))
app.use('/api/v1/meat/auth', createRateLimit({ windowMs: 15 * 60_000, max: 20 }))
app.use('/api/v1', createRateLimit({ max: 240 }))
app.use('/api/v1/auth', authRouter)
app.use('/api/v1/meat/auth', authRouter)
app.use('/api/v1', apiRouter)
app.use(structuredErrorHandler)
app.listen(port, () => console.log(`FoundMeat API listening on ${port}`))
