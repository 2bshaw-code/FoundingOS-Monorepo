...
// to core-intelligence/backend, so its build output must live under that
// subdirectory (core-intelligence/backend/api, .../public) rather than at the
...
const serviceRoot = path.join(root, 'core-intelligence/backend')

await build({
  entryPoints: [path.join(root, 'core-intelligence/backend/src/vercel-entry.ts')],
...
const generatedSrc = path.join(root, 'core-intelligence/backend/src/generated/prisma')
...
await writeFile(
  ...
  JSON.stringify({ name: 'core-intelligence-api-function', private: true, version: '0.0.0' }, null, 2),
)
...
await writeFile(
  ...
  '<!doctype html><title>Core.Intelligence API</title><p>This is an API-only service. See /health.</p>',
)

console.log('[build-core-intelligence-api] bundled api/index.js and copied generated Prisma client + bcrypt')
