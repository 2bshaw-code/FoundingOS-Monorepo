...
const founderPackages = [
  { slug: 'quantumos', name: 'QuantumOS', price: '£149/mo', description: 'The full FoundingOS command layer for leaders who want every brand, workflow, and AI decision in one view.', features: ['Portfolio command center', 'Globalisation controls', 'IntelligenceAI orchestration', 'Cross-brand reporting'] },
      ...
      <section className="hero">
        <div className="hero-copy">
          ...
          <Link href={founderPackageUrl(packagePlans[0]?.slug ?? 'quantumos')} className="btn btn-primary">Choose your package</Link>
            {isInternalHref(dashboardLink) ? <Link href={dashboardLink} className="btn btn-secondary">Chat with IntelligenceAI</Link> : <a href={dashboardLink} className="btn btn-secondary">Chat with IntelligenceAI</a>}
          </div>
        </div>
...
export { FoundingOSOnboarding } from './onboarding'
