...
function suggestedPrompts(brand: IntelligenceAIBrand, context: string) {
  const base = [
...
type IntelligenceAIBrand = Pick<BrandConsoleConfig, 'name' | 'accent'>

export function IntelligenceAI({ brand }: { brand: IntelligenceAIBrand }) {
  ...
  useEffect(() => {
    ...
    setMessages([
      {
        role: 'assistant',
        text: `Hi, I’m IntelligenceAI. I’m watching ${brand.name} ${context.toLowerCase()} and can help with next steps, risks, or quick actions.`,
      },
    ])
  ...
  return (
    <>
      <button
        ...
        aria-expanded={open}
        aria-label="Open IntelligenceAI"
      >
        <span>AI</span>
      </button>
      ...
      <aside className={`bob-panel ${open ? 'open' : ''}`} style={{ '--bob-accent': theme.accent, '--bob-glow': theme.glow } as React.CSSProperties} aria-hidden={!open}>
        ...
        <div className="bob-avatar bob-ai-circle">AI</div>
          <div>
            <strong>IntelligenceAI</strong>
            <span>{brand.name} · {context}</span>
          </div>
          <button type="button" className="bob-close" onClick={() => setOpen(false)} aria-label="Close IntelligenceAI">×</button>
        </header>
        ...
        <section className="bob-chat">
          ...
          {loading && <div className="bob-message assistant">IntelligenceAI is thinking…</div>}
        </section>
        ...
        <footer className="bob-compose">
          <textarea
            ...
            placeholder={`Ask IntelligenceAI about ${context.toLowerCase()}...`}
            rows={3}
          />
...
export const BobAI = IntelligenceAI
