...
const configs: Record<BusinessWorkspaceSlug, WorkspaceConfig> = {
  ...
  talent: {
    label: 'Talent', suite: 'Core.Workforce', accent: '#ff8a33', description: 'Recruit for your own business or run an agency: jobs, candidates, interviews, offers, checks, clients and placements.',
    subjects: ['Amara Johnson', 'Senior operator role', 'Harbour Cafe Group', 'Leo Osei', 'Warehouse lead role', 'Mei Chen reference', 'North & Co placement', 'Barista role'],
    metrics: [{ label: 'Open roles', value: '12', change: '4 priority hires' }, { label: 'Candidates', value: '184', change: '+28 this week' }, { label: 'Time to hire', value: '24d', change: '-3 days' }, { label: 'Placement fees', value: '£9.7k', change: '3 invoiced' }],
    modules: [module('overview', 'Recruitment home', 'Workspace'), module('jobs', 'Jobs', 'Hiring', ['Draft', 'Open', 'Interviewing', 'Filled']), module('candidates', 'Candidates', 'Hiring', ['Applied', 'Screening', 'Interview', 'Offer']), module('interviews', 'Interviews', 'Hiring', ['Planned', 'Confirmed', 'Complete', 'Decision']), module('offers', 'Offers', 'Hiring', ['Draft', 'Sent', 'Accepted', 'Onboarding']), module('references', 'References & checks', 'Hiring', ['Requested', 'Chasing', 'Received', 'Cleared']), module('talent-pool', 'Talent pool', 'Pipeline', ['Available', 'Contacted', 'Shortlisted', 'Placed']), module('clients', 'Agency clients', 'Agency', ['Prospect', 'Active', 'Hiring', 'Paused']), module('placements', 'Placements', 'Agency', ['Offered', 'Started', 'Invoiced', 'Paid']), module('reports', 'Recruitment reports', 'Intelligence'), module('automations', 'Automations', 'Intelligence'), module('team', 'Team & access', 'Administration'), module('integrations', 'Integrations', 'Administration'), module('security', 'Security & Access', 'Administration'), module('settings', 'Settings', 'Administration')],
  },
  hr: {
    label: 'HR', suite: 'Core.Workforce', accent: '#2ec4b6', description: 'Run the team you have: employees, contracts, rotas, timesheets, holiday, sickness, right-to-work, policies and payroll inputs.',
    subjects: ['Noah Williams review', 'September onboarding', 'Priya Anand contract', 'Week 14 timesheets', 'Saturday rota', 'Holiday policy', 'Kofi Mensah absence', 'October payroll'],
    metrics: [{ label: 'Headcount', value: '24', change: '+2 this month' }, { label: 'On shift today', value: '11', change: '1 gap to cover' }, { label: 'Absence rate', value: '2.1%', change: '-0.4pt' }, { label: 'Engagement', value: '82%', change: '+4 points' }],
    modules: [module('overview', 'People home', 'Workspace'), module('people', 'Employees', 'People'), module('onboarding', 'Onboarding', 'People', standard), module('contracts', 'Contracts', 'People', ['Draft', 'Sent', 'Signed', 'Ended']), module('right-to-work', 'Right to work', 'Compliance', ['Requested', 'Submitted', 'Checked', 'Verified']), module('documents', 'Documents', 'Compliance', ['Draft', 'Sent', 'Signed', 'Filed']), module('policies', 'Policies & handbook', 'Compliance', ['Draft', 'Review', 'Published', 'Archived']), module('rotas', 'Rotas & shifts', 'Time', ['Unfilled', 'Draft', 'Published', 'Worked']), module('timesheets', 'Timesheets', 'Time', ['Draft', 'Submitted', 'Approved', 'Paid']), module('time-off', 'Holiday', 'Time', ['Requested', 'Review', 'Approved', 'Complete']), module('sickness', 'Sickness & absence', 'Time', ['Off sick', 'Returned', 'Return-to-work done', 'Closed']), module('performance', 'Performance', 'Development', standard), module('learning', 'Learning', 'Development', standard), module('engagement', 'Engagement', 'Development'), module('payroll', 'Payroll inputs', 'Reward', ['Preparing', 'Review', 'Approved', 'Paid']), module('reports', 'HR reports', 'Intelligence'), module('automations', 'Automations', 'Intelligence'), module('team', 'Team & access', 'Administration'), module('integrations', 'Integrations', 'Administration'), module('security', 'Security & Access', 'Administration'), module('settings', 'Settings', 'Administration')],
  },
  health: {
...
const workspaceOrder: BusinessWorkspaceSlug[] = ['retail', 'logistics', 'finance', 'marketing', 'talent', 'hr', 'health', 'intelligence']
...
function ModuleKpiStrip({ kpis }: { kpis: ModuleKpi[] }) {
...
function RecordsPage({ autopilot, workspace, config, item, state, createRecord, advanceRecord, attachRecord, adjustStock, logNote, updateRecord, bulkAdvance, publishHandoff }: { autopilot?: AutopilotController; workspace: BusinessWorkspaceSlug; config: WorkspaceConfig; item: WorkspaceModule; state: WorkspaceState; createRecord: (module: string, record: WorkspaceRecord) => Promise<WorkspaceRecord>; advanceRecord: (module: string, record: WorkspaceRecord, status: string) => Promise<void>; attachRecord: (module: string, record: WorkspaceRecord, attachment: string | undefined, attachmentName: string) => void; adjustStock: (module: string, record: WorkspaceRecord, quantity: number, note: string) => void; logNote: (module: string, record: WorkspaceRecord, note: string, kind?: string) => void; updateRecord: (module: string, record: WorkspaceRecord, patch: Partial<Pick<WorkspaceRecord, 'name' | 'secondary' | 'value' | 'owner' | 'dueDate' | 'email' | 'phone'>>) => Promise<void>; bulkAdvance: (module: string, records: WorkspaceRecord[], status: string) => Promise<void>; publishHandoff: (module: string, record: WorkspaceRecord, target: BusinessWorkspaceSlug) => Promise<void> }) {
  ...
  const fields = profile?.fields ?? { name: 'Name', secondary: 'Detail', value: 'Value', owner: 'Owner' }
