...
import { brandList, brands } from '@foundingos/config'
...
const founderPackages = [
  { slug: 'quantumos', name: 'QuantumOS', price: '£149/mo', description: 'The full FounderOS command layer for leaders who want every brand, workflow, and AI decision in one view.', features: ['Portfolio command center', 'Globalisation controls', 'Bob AI orchestration', 'Cross-brand reporting'] },
...
export function Topbar({ brand }: { brand: BrandDefinition }) {
  return (
    <header className="topbar" style={consoleStyle(brand)}>
...
export function Sidebar({ brand }: { brand: BrandDefinition }) {
  ...
  return (
    <aside className="sidebar" style={consoleStyle(brand)}>
  ...
  return (
    <main className="site-shell" style={accentStyle}>
      ...
      <section className="hero">
        <div className="hero-copy">
          ...
          <Link href={founderPackageUrl(packagePlans[0]?.slug ?? 'quantumos')} className="btn btn-primary">Choose your package</Link>
            {isInternalHref(dashboardLink) ? <Link href={dashboardLink} className="btn btn-secondary">Chat with Bob AI</Link> : <a href={dashboardLink} className="btn btn-secondary">Chat with Bob AI</a>}
          </div>
        </div>

        <div className="hero-visual" aria-label={`${brand.name} overview`}>
          <div className="hero-panel" style={{ background: brandGradient(brand) }}>
      ...
      <section id="pricing" className="module-grid">
        {packagePlans.map((plan) => (
          <article key={plan.slug}>
            ...
            <Link className="btn btn-primary" href={founderPackageUrl(plan.slug)}>Open {plan.name}</Link>
          </article>
        ))}
      </section>
      ...
      <section id="contact" className="module-grid">
        <article>
          <h2>Connect with {brand.name}</h2>
          <p>Use the same social and messaging channels as the rest of the FounderOS ecosystem.</p>
...
export function FounderLauncher() {
  ...
  return (
    <main className="site-shell founder-shell" style={{ '--accent': '#4A90E2' } as React.CSSProperties}>
      <nav>
        <Link href="/">FounderOS</Link>
        <div className="site-nav-links">
          <a href={brands.foundingos.consoleUrl}>Console</a>
          <ThemeToggle />
        </div>
      </nav>
      <section className="hero">
        <div className="hero-copy">
          <p className="eyebrow">FounderOS</p>
        ...
        <div className="hero-visual" aria-label="FounderOS overview">
          <div className="hero-panel" style={{ background: brandGradient(brands.foundingos) }}>
      ...
      <section id="signup" className="module-grid">
        <article>
          <h2>Sign up your team</h2>
          <p>Pick the account method that suits the rollout and continue into FounderOS onboarding.</p>
          <div className="signup-grid">
            {signupOptions.map((option) => <a key={option.label} href={option.href} className="signup-chip">{option.label}</a>)}
          </div>
        </article>
      ...
      <section className="founder-bob-intro">
        ...
        <div className="founder-bob-copy">
          <p className="eyebrow">Bob AI</p>
          <h2>Meet Bob AI.</h2>
          <p>Bob AI — The Best Onboarding Bot in the World.</p>
          <p>Bob wasn’t created to be another chatbot. He wasn’t designed to be a gimmick, a feature, or a support tool. Bob was created to solve the single biggest problem in business software: people hate onboarding, people hate learning new systems, and people hate complicated dashboards.</p>
          <p>FounderOS fixes that by giving every user — from retail staff to meat suppliers, recruiters, IT teams, crypto traders, and founders — one universal guide who knows exactly what they need.</p>
          <p>Bob is simple, huge-capable, friendly, approachable, human-first, and a co-founder rather than a tool. He handles onboarding, setup, training, workflows, tasks, and questions instantly.</p>
          <p>Bob is the AI that makes FounderOS usable by SMEs, large companies, suppliers, retail staff, job seekers, recruiters, IT teams, crypto traders, and founders. He is the best onboarding bot in the world, and he is the heart of the entire FounderOS ecosystem.</p>
          <div className="hero-actions">
            {isInternalHref(consoleDashboardUrl(brands.foundingos))
              ? <Link className="btn btn-primary" href={consoleDashboardUrl(brands.foundingos)}>Meet Bob AI</Link>
              : <a className="btn btn-primary" href={consoleDashboardUrl(brands.foundingos)}>Meet Bob AI</a>}
          </div>
        </div>
      ...
      <section className="module-grid">
        {brandList.filter((brand) => brand.slug !== 'foundingos').map((brand) => (
          <article key={brand.slug}>
            <h2>{brand.name}</h2>
            <p>{brand.summary}</p>
            {isInternalHref(brand.consoleUrl)
              ? <Link className="btn btn-primary" href={brand.consoleUrl}>Open Console</Link>
              : <a className="btn btn-primary" href={brand.consoleUrl}>Open Console</a>}
          </article>
        ))}
      </section>
      <section id="contact" className="module-grid">
        <article>
          <h2>Connect with FounderOS</h2>
