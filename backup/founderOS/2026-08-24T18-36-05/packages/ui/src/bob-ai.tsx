...
function bobTheme(brand: BobBrand) {
  switch (brand.name) {
    case 'FoundRetail':
...
function suggestedPrompts(brand: BobBrand, context: string) {
  const base = [
  ...
  if (brand.name === 'FoundIT') return ['Show system alerts', 'Summarise data pipeline health', 'Create a ticket', 'Audit assets']
...
function smartActions(brand: BobBrand, context: string) {
...
type BobBrand = Pick<BrandConsoleConfig, 'name' | 'accent'> | Pick<BrandDefinition, 'name' | 'accent'>

export function BobAI({ brand }: { brand: BobBrand }) {
  ...
  const theme = useMemo(() => bobTheme(brand), [brand])
  ...
  useEffect(() => {
    ...
    setMessages([
      {
        role: 'assistant',
        text: `Hi, I’m Bob. I’m watching ${brand.name} ${context.toLowerCase()} and can help with next steps, risks, or quick actions.`,
      },
    ])
  ...
  return (
    <>
      <button
        type="button"
        className="bob-fab bob-ai-circle"
        style={{ '--bob-accent': theme.accent, '--bob-glow': theme.glow } as React.CSSProperties}
      ...
      <aside className={`bob-panel ${open ? 'open' : ''}`} style={{ '--bob-accent': theme.accent, '--bob-glow': theme.glow } as React.CSSProperties} aria-hidden={!open}>
        <header className="bob-panel-header">
        <div className="bob-avatar bob-ai-circle">B</div>
          <div>
            <strong>Bob AI</strong>
            <span>{brand.name} · {context}</span>
          </div>
          <button type="button" className="bob-close" onClick={() => setOpen(false)} aria-label="Close Bob AI">×</button>
        </header>

        <section className="bob-chat">
          {messages.map((message, index) => (
            <div key={`${message.role}-${index}`} className={`bob-message ${message.role}`}>
              {message.text}
            </div>
          ))}
          {loading && <div className="bob-message assistant">Bob is thinking…</div>}
        </section>

        <section className="bob-prompts">
          <h3>Suggested prompts</h3>
          <div className="bob-chip-grid">
            {prompts.map((prompt) => (
              <button key={prompt} type="button" className="bob-chip" onClick={() => submit(prompt)}>{prompt}</button>
            ))}
          </div>
        </section>

        <section className="bob-actions">
          <h3>Smart actions</h3>
        ...
        <footer className="bob-compose">
          <textarea
            value={input}
            onChange={(event) => setInput(event.target.value)}
            placeholder={`Ask Bob about ${context.toLowerCase()}...`}
            rows={3}
          />
          <button type="button" className="btn btn-primary" onClick={() => submit(input)}>Send</button>
        </footer>
      </aside>
    </>
  )
