...
export function ModuleHeader({ config, title, description }: { config: BrandConsoleConfig; title: string; description: string }) {
  return <header className="module-header" style={consoleStyle(config)}><p>{bobLabel(config)}</p><h1>{title}</h1><span>{description}</span></header>
}
    ...
    <section className="console-page" style={accentStyle}>
      ...
      <div className="module-card-grid">
        {moduleCards.map((module, index) => (
          <Link key={module.href} className="module-card" href={module.href}>
      ...
      <div className="console-grid">
        ...
        <article className="panel">
          <h2>Quick actions</h2>
          <div className="action-list">{config.quickActions.map((action) => <button key={action} type="button">{action}</button>)}</div>
        </article>
        <article className="panel">
          <h2>CRM summary</h2>
          <p>{crm.summary}</p>
...
function extensionGroups(config: BrandConsoleConfig) {
  const shared = [
    ...
    },
    {
      ...
      rows: makeRows('activity', [`${config.name} workspace opened`, 'Subscription checked', 'Module permissions refreshed', 'Bob AI reviewed context'], [
        { key: 'name', label: 'Event' },
    ...
    <section className="console-page" style={accentStyle}>
      ...
      <div className="settings-grid">
        {config.settings.map((setting) => (
          <article key={setting} className="panel">
            <h2>{setting}</h2>
      ...
      <div className="panel color-preview">
        <h2>Brand colors</h2>
    ...
    <section className="console-page" style={accentStyle}>
      ...
      <div className="module-card-grid">
        ...
        <article className="module-card module-card-static">
          ...
          <p>{config.name === 'FoundRetail' ? 'Customers, suppliers, and stores.' : config.name === 'FoundMeat' ? 'Farms, processors, logistics partners, and products.' : config.name === 'FoundIT' ? 'Clients, systems, and integrations.' : config.name === 'FoundTalent' ? 'Candidates, employers, jobs, and intelligence.' : 'Wallets, exchanges, triggers, and portfolio intelligence.'}</p>
        </article>
      </div>
...
export function BrandDashboardCardLink({ brand, module }: { brand: BrandConsoleConfig; module: BrandModule }) {
  return <Link href={moduleHref(module.id)} className="module-card" style={consoleStyle(brand)}>{module.label}</Link>
}
...
const packageCatalog: Record<string, BrandPackage[]> = {
  FounderOS: [
    { slug: 'quantumos', name: 'QuantumOS', price: '£149/mo', description: 'The full FounderOS command layer for leaders who want every brand, workflow, and AI decision in one view.', features: ['Portfolio command center', 'Globalisation controls', 'Bob AI orchestration', 'Cross-brand reporting'], benefits: ['See every brand at once', 'Standardise operations', 'Move faster with AI guidance'], audience: 'Best for founders and operators running the full ecosystem.' },
