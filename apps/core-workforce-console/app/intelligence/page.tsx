...
export default async function IntelligencePage() {
  const layer = BRAND_PERSONALITIES.talent
  const ai = generateBrandAIOutput('talent')
  const quantum = await enrichBrandSignalWithQuantum(buildBrandSignal('talent'))
  return (
    <>
      <BrandMicroDashboard
        brandSlug="talent"
      ...
      <AIInsightsPanel
        ...
        sourceScope={['talent', 'finance']}
      />
      <InsightsPanel apiBase={CORE_OPERATIONS_API_BASE} sourceScope={['talent', 'finance']} />
    </>
  )
}
