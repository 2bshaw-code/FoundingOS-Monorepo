...
// Local, best-effort survey log for the FoundRetail brand website's tester survey pages.
// This intentionally mirrors the existing dormant-safe pattern used for legal-acceptance
// logging elsewhere in the ecosystem: on serverless platforms the filesystem is often
// read-only/ephemeral, so a write failure here must never break the submit flow for the
// person filling in the survey — it's wrapped in try/catch and always resolves ok.
...
// (server-to-server, so it's never subject to browser CORS) so SuperDash can show real
// cross-app tester activity, category health, anomalies, Guardian warnings, and autonomous
// suggestions. Best-effort: if foundingos-console is unreachable, the tester's local
// submission above has already succeeded and this never blocks their experience.
//
...
// production builds, which is correct for browser-facing links (never point a visitor at a
// dead localhost link) but wrong here: a server-to-server fetch needs a real absolute URL
// in every environment, local or deployed.
