...
const consoleLinks = [
  { name: 'Starter Console', href: 'http://127.0.0.1:4009', target: '_blank', rel: 'noopener noreferrer' },
  { name: 'Growth Console', href: 'http://127.0.0.1:4010', target: '_blank', rel: 'noopener noreferrer' },
] as const

const cryptoHomeUrl = brands.crypto.webUrl
const cryptoPackageUrl = (slug: string) => `${brands.crypto.consoleUrl.replace(/\/+$/, '')}/console/packages/${slug}`
...
export default function Page() {
  ...
  const consoleDetails = [
    { name: 'Crypto Intelligence & Auto-Execution Console', href: `${brands.crypto.consoleUrl.replace(/\/+$/, '')}/console`, description: 'Track market signals, execution context, and automation from one control surface.' },
  ] as const
    ...
    <main style={{ minHeight: '100vh', background: '#070c14', color: '#f5f3ff', fontFamily: 'Inter, Segoe UI, sans-serif' }}>
      <header style={{ position: 'sticky', top: 0, zIndex: 20, background: 'rgba(7,12,20,0.75)', backdropFilter: 'blur(16px)', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 20px', minHeight: 72, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 20 }}>
          <a href="#top" style={{ display: 'flex', alignItems: 'center', gap: 12, textDecoration: 'none', color: '#f5f3ff' }}>
            ...
            <div>
              <strong style={{ display: 'block', fontSize: 22, lineHeight: 1.1 }}>FoundCrypto</strong>
          ...
          <nav style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 10, justifyContent: 'flex-end' }}>
            ...
            <details style={{ position: 'relative', display: 'inline-block' }}>
              ...
              <div style={{ position: 'absolute', right: 0, top: 'calc(100% + 10px)', minWidth: 280, background: 'rgba(8,13,20,0.98)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: 12, boxShadow: '0 20px 40px rgba(0,0,0,0.35)', display: 'grid', gap: 8, zIndex: 30 }}>
                {consoleLinks.map(({ name, href, target, rel }) => (
                  <a key={name} href={href} target={target} rel={rel} style={{ color: '#f5f3ff', textDecoration: 'none', padding: '10px 12px', borderRadius: 10, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', fontSize: 13, fontWeight: 700 }}>{name}</a>
                ))}
              </div>
            </details>
      ...
      <section id="top" style={{ position: 'relative', overflow: 'hidden', background: 'radial-gradient(circle at top left, rgba(156,39,176,0.12), transparent 30%), #0d0b17' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '72px 20px 52px', display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: 28, alignItems: 'center' }}>
          <div>
            <p style={{ margin: 0, color: '#d8b4fe', fontSize: 12, letterSpacing: '0.18em', textTransform: 'uppercase', fontWeight: 800 }}>FoundCrypto brand ecosystem</p>
            ...
            <p style={{ marginTop: 22, maxWidth: 620, color: '#ddd6fe', fontSize: 20, lineHeight: 1.8 }}>{'FoundCrypto connects charting, alerts, execution context, and market intelligence across all core channels.'}</p>
        ...
        <div style={{ border: '1px solid rgba(255,255,255,0.08)', borderRadius: 28, background: 'linear-gradient(135deg, rgba(156,39,176,0.12), rgba(13,18,22,0.8))', padding: 28 }}>
          <div style={{ minHeight: 320, borderRadius: 24, background: 'radial-gradient(circle at top, rgba(156,39,176,0.28), transparent 58%), rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', display: 'grid', placeItems: 'center', padding: 20, textAlign: 'center' }}>
            <div style={{ display: 'grid', gap: 14, justifyItems: 'center', width: '100%' }}>
              ...
              <div>
                <div style={{ letterSpacing: '0.18em', textTransform: 'uppercase', fontSize: 12, fontWeight: 800, color: '#ddd6fe' }}>Brand Intelligence AI</div>
      ...
      <section id="contact" style={{ maxWidth: 1200, margin: '0 auto', padding: '24px 20px 12px' }}>
        <div style={{ border: '1px solid rgba(255,255,255,0.08)', borderRadius: 22, background: 'rgba(255,255,255,0.03)', padding: 22, display: 'grid', gap: 14 }}>
          ...
          <h3 style={{ margin: 0, fontSize: 'clamp(26px, 3.4vw, 38px)' }}>Connect with FoundCrypto on every channel.</h3>
        </div>
      </section>
      <footer style={{ borderTop: '1px solid rgba(255,255,255,0.08)', background: '#070c14', marginTop: 60 }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '28px 20px 40px', display: 'flex', flexWrap: 'wrap', gap: 20, justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <strong style={{ fontSize: 22 }}>{'FoundCrypto'}</strong>
          ...
          <div style={{ display: 'grid', gap: 10, justifyItems: 'start' }}>
            <div style={{ color: '#ddd6fe' }}>hello@FoundCrypto.com</div>
            ...
            <div style={{ color: '#ddd6fe', fontSize: 14 }}>© 2026 FoundCrypto • All rights reserved</div>
          </div>
          ...
          <div onClick={() => setBrandIntelligenceOpen(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(1, 5, 10, 0.72)', display: 'grid', placeItems: 'center', padding: 20, zIndex: 60 }}>
            <div onClick={(event) => event.stopPropagation()} style={{ position: 'relative', width: 'min(440px, 92vw)', background: 'linear-gradient(180deg, rgba(15, 23, 42, 0.98), rgba(10, 15, 20, 0.98))', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 28, boxShadow: '0 28px 80px rgba(0,0,0,0.45)', padding: 24, color: '#edf7ff' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 14 }}>
                ...
                <div>
                  <div style={{ letterSpacing: '0.16em', textTransform: 'uppercase', fontSize: 11, fontWeight: 800, color: '#9C27B0' }}>Brand Intelligence AI</div>
                  <h3 style={{ margin: '8px 0 0', fontSize: 24 }}>Your smart brand assistant</h3>
                </div>
              </div>
              <p style={{ margin: 0, color: '#bfd8ee', lineHeight: 1.7 }}>Brand Intelligence AI helps your market team track signal, risk thresholds, and execution context so decisions stay sharp, fast, and well-informed.</p>
