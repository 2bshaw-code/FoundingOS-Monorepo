...
const pricing = [
  { slug: 'supportos', name: 'StarterPack', price: '£19.99', blurb: 'For lean teams starting with cleaner workflows and faster channel responses.', features: ['1 workspace', 'Core automation', 'Multi-channel messaging', 'Basic reporting'], featured: false },
  { slug: 'networkos', name: 'Growth', price: '£49.99', blurb: 'For teams scaling with more complexity, more volume, and stronger operational visibility.', features: ['Up to 5 workspaces', 'Smart routing', 'Advanced reporting', 'Team coordination'], featured: true },
  { slug: 'enterpriseos', name: 'Enterprise', price: '£99.99', blurb: 'For multi-team operations that need premium automation, governance, and control.', features: ['Unlimited workspaces', 'Advanced automation', 'Governance controls', 'Priority support'], featured: false },
] as const
...
const itPackageUrl = (slug: string) => `${brands.foundthat.consoleUrl.replace(/\/+$/, '')}/console/packages/${slug}`
...
export default function Page() {
  ...
  const foundAiQuestions = [
    'How can I explain local discovery simply?',
    'What packages or services should customers see?',
    'Can you show me the lead workflow?',
    'Which console handles data operations?',
  ] as const
  const foundAiActions = [
    { label: 'Review customer demand', answer: 'Your strongest demand is coming from the mid-market segment, and the next wave of leads is most likely to convert with a faster response and a tailored offer.' },
    { label: 'Prioritise lead routing', answer: 'I’ve ranked the new leads by urgency and fit, and I’ve recommended the best handoff path for your sales team.' },
    { label: 'Surface common blockers', answer: 'The main delay in this pipeline is slow follow-up and inconsistent qualification; I’ve prepared a workflow to reduce friction.' },
  ...
  return (
    <main className="quantum-sig-foundthat" style={{ minHeight: '100vh', background: '#070c14', color: '#f8fafc', fontFamily: 'Inter, Segoe UI, sans-serif' }}>
      <header className="quantum-header" style={{ position: 'sticky', top: 0, zIndex: 20, background: 'rgba(7,12,20,0.75)', backdropFilter: 'blur(16px)', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 20px', minHeight: 72, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 20 }}>
          <a href="#top" style={{ display: 'flex', alignItems: 'center', gap: 12, textDecoration: 'none', color: '#f8fafc' }}>
            ...
            <div>
              <strong style={{ display: 'block', fontSize: 22, lineHeight: 1.1 }}>Intelligence.ITOps</strong>
          ...
          <nav style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 10, justifyContent: 'flex-end' }}>
            ...
            <QuantumConsoleEntry brandName={brands.foundthat.name} glyph="✦" starterUrl={brands.foundthat.starterConsoleUrl} growthUrl={brands.foundthat.consoleUrl} />
            ...
            <a href={itHomeUrl} className="quantum-link-glow" style={navChip}>Back to home</a>
            <a href="/legal" className="quantum-link-glow" style={navChip}>Legal</a>
          </nav>
        </div>
      </header>

      <section id="top" style={{ position: 'relative', overflow: 'hidden', background: 'radial-gradient(circle at top left, rgba(255,221,0,0.12), transparent 30%), #0b1321' }}>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '72px 20px 52px', display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: 28, alignItems: 'center' }}>
          <div>
            <p style={{ margin: 0, color: '#fef3c7', fontSize: 12, letterSpacing: '0.18em', textTransform: 'uppercase', fontWeight: 800 }}>Intelligence.ITOps — part of FoundingOS</p>
            ...
            <p style={{ marginTop: 22, maxWidth: 620, color: '#dfeaf8', fontSize: 20, lineHeight: 1.8 }}>{'Intelligence.ITOps connects demand, opportunity, enquiry routing, and reporting across every market channel that matters.'}</p>

            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 18, marginTop: 28 }}>
              <PackageModal tier={pricing.find((t) => t.slug === 'supportos')!} packageUrl={itPackageUrl('supportos')} accent={brands.foundthat.accent} buttonLabel="Choose your package" buttonStyle={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', minHeight: 50, padding: '0 22px', borderRadius: 12, background: '#FFDD00', color: '#061018', textDecoration: 'none', fontWeight: 900 }} />
              <a href="#features" className="quantum-btn" style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', minHeight: 50, padding: '0 22px', borderRadius: 12, background: 'rgba(255,255,255,0.05)', color: '#f8fafc', textDecoration: 'none', fontWeight: 900, border: '1px solid rgba(255,255,255,0.08)' }}>View features</a>
            </div>
          </div>
          ...
          <div style={{ border: '1px solid rgba(255,255,255,0.08)', borderRadius: 24, background: 'linear-gradient(180deg, rgba(255,255,255,0.05), rgba(15,23,32,0.82))', padding: 24, boxShadow: '0 26px 70px rgba(0,0,0,0.25)' }}>
            <div style={{ minHeight: 320, borderRadius: 24, background: 'radial-gradient(circle at top, rgba(255,221,0,0.18), transparent 58%), rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', padding: 22, display: 'grid', gap: 14 }}>
              {['Operations visibility', 'Multi-channel messaging', 'Automation workflow', 'Brand reporting'].map((item) => (
                <div key={item} style={{ display: 'flex', justifyContent: 'space-between', gap: 12, borderTop: '1px solid rgba(255,255,255,0.08)', paddingTop: 12 }}>
                  ...
                  <span style={{ color: '#FFDD00', fontWeight: 800 }}>Live</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <section id="features" style={{ maxWidth: 1200, margin: '0 auto', padding: '72px 20px 24px' }}>
        <div style={{ display: 'grid', gap: 10, marginBottom: 24 }}>
          <p style={{ margin: 0, color: '#FFDD00', fontSize: 12, letterSpacing: '0.18em', textTransform: 'uppercase', fontWeight: 800 }}>Features</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 18 }}>
          {features.map((feature) => (
            <article key={feature.title} className="quantum-card" style={{ padding: 22, borderRadius: 18, border: '1px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.02)', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.04)' }}>
              ...
              <div className="quantum-icon-pulse" style={{ width: 42, height: 42, borderRadius: 12, background: 'rgba(255,221,0,0.12)', color: '#FFDD00', display: 'grid', placeItems: 'center', fontWeight: 900 }}>{feature.icon}</div>
              <h3 style={{ margin: '18px 0 10px', fontSize: 26 }}>{feature.title}</h3>
              <p style={{ margin: 0, lineHeight: 1.7, color: '#dfeaf8' }}>{feature.description}</p>
            </article>
          ))}
        </div>
      </section>
      ...
      <section id="found-ai" style={{ maxWidth: 1200, margin: '0 auto', padding: '52px 20px 24px' }}>
        <div style={{ border: '1px solid rgba(255,255,255,0.08)', borderRadius: 28, background: 'linear-gradient(135deg, rgba(255,221,0,0.12), rgba(13,18,22,0.8))', padding: 28 }}>
          <div style={{ minHeight: 320, borderRadius: 24, background: 'radial-gradient(circle at top, rgba(255,221,0,0.28), transparent 58%), rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', display: 'grid', placeItems: 'center', padding: 20, textAlign: 'center' }}>
            <div style={{ display: 'grid', gap: 14, justifyItems: 'center', width: '100%' }}>
              <div style={{ width: 132, height: 132, borderRadius: '50%', background: 'radial-gradient(circle at 30% 30%, rgba(255,255,255,0.34), transparent 38%), #FFDD00', color: '#061018', display: 'grid', placeItems: 'center', fontSize: 56, fontWeight: 900, boxShadow: '0 0 0 14px rgba(255,255,255,0.04), 0 0 44px rgba(255,221,0,0.45)', animation: 'foundAiFloat 4s ease-in-out infinite' }}>AI</div>
              <div>
                <div style={{ letterSpacing: '0.18em', textTransform: 'uppercase', fontSize: 12, fontWeight: 800, color: '#fef08a' }}>FoundAI</div>
                <h3 style={{ margin: '10px 0 0', fontSize: 28, lineHeight: 1.1 }}>Premium support for modern discovery conversations</h3>
                <p style={{ margin: '12px auto 0', maxWidth: 320, color: '#dfeaf8', lineHeight: 1.7 }}>Instant answers, clearer guidance, and a more human customer experience across every discovery journey.</p>
              </div>
              <div style={{ display: 'grid', gap: 10, width: '100%', maxWidth: 620 }}>
                {foundAiHighlights.map((item) => (
                  <div key={item} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', borderRadius: 14, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', color: '#f8fafc', fontWeight: 700 }}>
                    <span style={{ width: 10, height: 10, borderRadius: '50%', background: '#FFDD00', boxShadow: '0 0 18px rgba(255,221,0,0.55)' }} />
                    {item}
                  </div>
                ))}
              </div>
              <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 18, marginTop: 8 }}>
                ...
                <PackageModal tier={pricing.find((t) => t.slug === 'supportos')!} packageUrl={itPackageUrl('supportos')} accent={brands.foundthat.accent} buttonLabel="Choose your package" buttonStyle={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', minHeight: 50, padding: '0 22px', borderRadius: 12, background: 'rgba(255,255,255,0.05)', color: '#f8fafc', textDecoration: 'none', fontWeight: 900, border: '1px solid rgba(255,255,255,0.08)' }} />
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="messaging" style={{ maxWidth: 1200, margin: '0 auto', padding: '52px 20px 24px' }}>
        <div style={{ display: 'grid', gap: 10, marginBottom: 24 }}>
          <p style={{ margin: 0, color: '#FFDD00', fontSize: 12, letterSpacing: '0.18em', textTransform: 'uppercase', fontWeight: 800 }}>Messaging Channels</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 14 }}>
          {messagingOptions.map((channel) => (
            <div key={channel} style={{ border: '1px solid rgba(255,255,255,0.08)', borderRadius: 18, padding: 18, background: 'rgba(255,255,255,0.02)' }}>
              <div style={{ width: 54, height: 54, borderRadius: 14, background: 'rgba(255,221,0,0.12)', color: '#FFDD00', display: 'grid', placeItems: 'center', fontWeight: 900 }}>{channel.slice(0, 2).toUpperCase()}</div>
      ...
      <section id="unified-messaging" style={{ maxWidth: 1200, margin: '0 auto', padding: '24px 20px 24px' }}>
        <DemoMessageBoard config={{ name: brands.foundthat.name, logo: brands.foundthat.logo }} variant="preview" />
      </section>

      <section id="pricing" style={{ maxWidth: 1200, margin: '0 auto', padding: '52px 20px 24px' }}>
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          <p style={{ margin: 0, color: '#FFDD00', fontSize: 12, letterSpacing: '0.18em', textTransform: 'uppercase', fontWeight: 800 }}>Pricing</p>
          <h3 style={{ margin: '16px 0 0', fontSize: 'clamp(32px, 4vw, 46px)' }}>Choose your package</h3>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 18 }}>
          {pricing.map((tier) => (
            <article key={tier.name} style={{ padding: 24, borderRadius: 18, border: tier.featured ? '1px solid ' + '#FFDD00' : '1px solid rgba(255,255,255,0.08)', background: tier.featured ? 'linear-gradient(180deg, rgba(255,221,0,0.12), rgba(10,15,20,0.85))' : 'rgba(255,255,255,0.02)', display: 'grid', gap: 12 }}>
              <div>
                <p style={{ margin: 0, color: '#FFDD00', fontSize: 12, letterSpacing: '0.14em', textTransform: 'uppercase', fontWeight: 800 }}>{tier.name}</p>
                <h4 style={{ margin: '10px 0 0', fontSize: 42 }}>{tier.price}</h4>
              </div>
              <p style={{ margin: 0, color: '#dfeaf8', lineHeight: 1.7 }}>{tier.blurb}</p>
              <ul style={{ margin: 0, paddingLeft: 18, color: '#f8fafc', lineHeight: 1.9 }}>
                {tier.features.map((feature) => <li key={feature}>{feature}</li>)}
              </ul>
              <PackageModal tier={tier} packageUrl={itPackageUrl(tier.slug)} accent={brands.foundthat.accent} buttonLabel="Get started" buttonStyle={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', minHeight: 46, borderRadius: 12, background: '#FFDD00', color: '#071014', fontWeight: 900, textDecoration: 'none' }} />
            </article>
          ))}
        </div>
      </section>
      <section id="signup" style={{ maxWidth: 1200, margin: '0 auto', padding: '8px 20px 24px' }}>
        <div style={{ border: '1px solid rgba(255,255,255,0.08)', borderRadius: 22, background: 'rgba(255,255,255,0.03)', padding: 22, display: 'grid', gap: 14 }}>
          <p style={{ margin: 0, color: '#FFDD00', fontSize: 12, letterSpacing: '0.18em', textTransform: 'uppercase', fontWeight: 800 }}>Signup</p>
      ...
      <section id="contact" style={{ maxWidth: 1200, margin: '0 auto', padding: '24px 20px 12px' }}>
        <div style={{ border: '1px solid rgba(255,255,255,0.08)', borderRadius: 22, background: 'rgba(255,255,255,0.03)', padding: 22, display: 'grid', gap: 14 }}>
          <p style={{ margin: 0, color: '#FFDD00', fontSize: 12, letterSpacing: '0.18em', textTransform: 'uppercase', fontWeight: 800 }}>Contact</p>
          <h3 style={{ margin: 0, fontSize: 'clamp(26px, 3.4vw, 38px)' }}>Connect with Intelligence.ITOps on every channel.</h3>
        </div>
      </section>
      <footer style={{ borderTop: '1px solid rgba(255,255,255,0.08)', background: '#070c14', marginTop: 60 }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '28px 20px 40px', display: 'flex', flexWrap: 'wrap', gap: 20, justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <strong style={{ fontSize: 22 }}>{'Intelligence.ITOps'}</strong>
          ...
          <div style={{ display: 'grid', gap: 10, justifyItems: 'start' }}>
            <div style={{ color: '#dfeaf8' }}>hello@foundthat.com</div>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              <a href="/legal" style={{ color: '#FFDD00', textDecoration: 'none', fontWeight: 700 }}>Privacy Policy</a>
              <a href="/legal" style={{ color: '#FFDD00', textDecoration: 'none', fontWeight: 700 }}>Terms</a>
              <a href="/legal" style={{ color: '#FFDD00', textDecoration: 'none', fontWeight: 700 }}>Cookies</a>
            </div>
            <div style={{ color: '#dfeaf8', fontSize: 14 }}>© 2026 Intelligence.ITOps • All rights reserved</div>
          </div>
          <div style={{ width: '100%', marginTop: 8 }}>
            <PremiumSocialLinks accent="#FFDD00" mode="full" label="Social & messaging" />
          </div>
        </div>
      </footer>
      ...
      <button
        ...
        style={{ position: 'fixed', right: 22, bottom: 22, zIndex: 65, width: 90, height: 90, borderRadius: '50%', border: 'none', background: 'radial-gradient(circle at 30% 30%, rgba(255,255,255,0.26), transparent 42%), #FFDD00', color: '#061018', boxShadow: '0 18px 40px rgba(0,0,0,0.3), 0 0 0 1px rgba(255,255,255,0.18), 0 0 28px rgba(255,221,0,0.45)', cursor: 'pointer', animation: 'foundAiFloat 4s ease-in-out infinite, foundAiGlow 2.8s ease-in-out infinite' }}
      >
          ...
          <div onClick={() => setFoundAiOpen(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(1, 5, 10, 0.72)', display: 'grid', placeItems: 'center', padding: 20, zIndex: 60 }}>
            <div onClick={(event) => event.stopPropagation()} style={{ position: 'relative', width: 'min(440px, 92vw)', background: 'linear-gradient(180deg, rgba(15, 23, 42, 0.98), rgba(10, 15, 20, 0.98))', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 28, boxShadow: '0 28px 80px rgba(0,0,0,0.45)', padding: 24, color: '#edf7ff' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 14 }}>
                <div style={{ width: 74, height: 74, borderRadius: '50%', background: '#FFDD00', color: '#071014', display: 'grid', placeItems: 'center', fontWeight: 900, fontSize: 28, boxShadow: '0 0 0 8px rgba(255,255,255,0.04)', animation: 'foundAiFloat 4s ease-in-out infinite' }}>AI</div>
                <div>
                  <div style={{ letterSpacing: '0.16em', textTransform: 'uppercase', fontSize: 11, fontWeight: 800, color: '#FFDD00' }}>FoundAI</div>
                  <h3 style={{ margin: '8px 0 0', fontSize: 24 }}>Your smart brand assistant</h3>
                </div>
              </div>
              <p style={{ margin: 0, color: '#bfd8ee', lineHeight: 1.7 }}>FoundAI helps your discovery team qualify leads, route enquiries, and act on high-intent opportunities before they slow down.</p>
              ...
              <div style={{ marginTop: 18, borderRadius: 16, border: '1px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.02)', minHeight: 100, padding: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {foundAiLoading ? (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, color: '#FFDD00', fontWeight: 700 }}>
                    <span style={{ display: 'inline-flex', gap: 6, alignItems: 'center' }}>
                      <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#FFDD00', animation: 'foundAiPulse 1s ease-in-out infinite' }} />
                      <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#FFDD00', animation: 'foundAiPulse 1s ease-in-out 0.15s infinite' }} />
                      <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#FFDD00', animation: 'foundAiPulse 1s ease-in-out 0.3s infinite' }} />
                    </span>
      ...
      <style jsx>{`
        ...
        @keyframes foundAiGlow {
          0%, 100% { transform: translateY(0) scale(1); box-shadow: 0 18px 40px rgba(0,0,0,0.3), 0 0 0 1px rgba(255,255,255,0.18), 0 0 28px rgba(255,221,0,0.45); }
          50% { transform: translateY(-2px) scale(1.03); box-shadow: 0 22px 48px rgba(0,0,0,0.32), 0 0 0 1px rgba(255,255,255,0.22), 0 0 34px rgba(255,221,0,0.62); }
        }
...
const navChip = {
  display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '6px 2px', color: '#f8fafc', textDecoration: 'none', fontSize: 13, fontWeight: 700,
}
