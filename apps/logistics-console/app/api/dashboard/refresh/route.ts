...
// Full Demo Mode: read-only, deterministic combined dashboard metrics for Logistics. No DB
// writes, no external network calls, no secrets — safe to call from a Vercel cron every
// 5 minutes.
export async function GET() {
  const windowMs = 5 * 60 * 1000
  const activeUsers = Math.floor(seededValue('logistics:activeUsers', windowMs, 40, 0.5))
  const ordersToday = Math.floor(seededValue('logistics:ordersToday', windowMs, 120, 0.4))
  const revenueTodayUsd = Number(seededValue('logistics:revenueToday', windowMs, 4800, 0.35).toFixed(2))
  const openAlerts = Math.floor(hashSeed(`logistics:alerts:${timeBucket(windowMs)}`) * 5)
