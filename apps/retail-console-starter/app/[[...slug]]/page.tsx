...
export default async function Page({ params }: { params: Promise<{ slug?: string[] }> }) { const { slug = [] } = await params; if (slug.length === 0) return <ConsoleDashboard brand={brands.retail} />; if (slug[0] === 'dashboard') return <ConsoleDashboard brand={brands.retail} />; if (slug[0] === 'settings') return <SettingsPage brand={brands.retail} />; if (slug[0] === 'modules' && slug[1]) return <ModulePage brand={brands.retail} moduleId={slug[1]} />; notFound() }