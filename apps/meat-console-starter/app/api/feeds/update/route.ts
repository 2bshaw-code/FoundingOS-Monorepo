...
const CATALOGUE = ['Grass-Fed Ribeye', 'Free-Range Chicken Thighs', 'Dry-Aged Sirloin', 'Smoked Bacon', 'Ground Beef 80/20']

// Full Demo Mode: read-only, deterministic mock product feed for Meat. No paid APIs, no
// DB writes — safe to call from a Vercel cron every 20 minutes.
export async function GET() {
  const windowMs = 20 * 60 * 1000
  const bucket = timeBucket(windowMs)
  const products = CATALOGUE.map((name, i) => {
    const id = `meat-feed-${i + 1}`
