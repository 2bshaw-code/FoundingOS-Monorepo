...
const pricing = [
  { slug: 'traderos', name: 'Starter', price: '£19.99', blurb: 'For lean teams starting with cleaner workflows and faster channel responses.', features: ['1 workspace', 'Core automation', 'Multi-channel messaging', 'Basic reporting'], featured: false },
...
const consoleLinks = [
  { name: 'Logistics Fleet & Route Console', href: `${brands.logistics.consoleUrl.replace(/\/+$/, '')}/console` },
] as const

const cryptoHomeUrl = brands.logistics.webUrl
const cryptoPackageUrl = (slug: string) => `${brands.logistics.consoleUrl.replace(/\/+$/, '')}/console/packages/${slug}`
...
export default function Page() {
  ...
  const bobQuestions = [
    'How do I review fleet routes?',
    'What should I watch before dispatch?',
    'Can you explain delivery risk simply?',
    'Which console manages warehousing?',
  ] as const
  const bobActions = [
    { label: 'Check fleet risk', answer: 'Momentum is improving across your top routes, but one high-delay corridor is flashing elevated risk and should be reviewed before the next dispatch window.' },
    { label: 'Prioritise delivery triage', answer: 'I’ve sorted the most relevant deliveries and highlighted which need immediate attention versus which can wait.' },
    { label: 'Prepare a routing brief', answer: 'Your strongest route is a measured capacity increase on the busiest corridor, paired with a tighter risk guard for the volatile segment.' },
    { label: 'Review dispatch pressure', answer: 'Dispatch risk is building around the next peak window, so I’ve flagged a tighter routing plan and a fresh briefing for the team.' },
  ] as const
  const consoleDetails = [
    { name: 'Logistics Fleet & Route Console', href: `${brands.logistics.consoleUrl.replace(/\/+$/, '')}/console`, description: 'Track fleet status, route optimisation, and delivery signals from one control surface.' },
  ] as const
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#070c14', color: '#f5f3ff', fontFamily: 'Inter, Segoe UI, sans-serif' }}>
            ...
            <details style={{ position: 'relative', display: 'inline-block' }}>
              ...
              <div style={{ position: 'absolute', right: 0, top: 'calc(100% + 10px)', minWidth: 280, background: 'rgba(8,13,20,0.98)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: 12, boxShadow: '0 20px 40px rgba(0,0,0,0.35)', display: 'grid', gap: 8, zIndex: 30 }}>
                {consoleLinks.map(({ name, href }) => (
                  <a key={name} href={href} style={{ color: '#f5f3ff', textDecoration: 'none', padding: '10px 12px', borderRadius: 10, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', fontSize: 13, fontWeight: 700 }}>{name}</a>
                ))}
              </div>
            </details>
            <a href={cryptoHomeUrl} style={navChip}>Back to home</a>
            <a href="/legal" style={navChip}>Legal</a>
          </nav>
        </div>
      </header>

      <section id="top" style={{ position: 'relative', overflow: 'hidden', background: 'radial-gradient(circle at top left, rgba(156,39,176,0.12), transparent 30%), #0d0b17' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '72px 20px 52px', display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: 28, alignItems: 'center' }}>
          <div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 18, marginTop: 28 }}>
              ...
              <a href="#features" style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', minHeight: 50, padding: '0 22px', borderRadius: 12, background: 'rgba(255,255,255,0.05)', color: '#f5f3ff', textDecoration: 'none', fontWeight: 900, border: '1px solid rgba(255,255,255,0.08)' }}>View features</a>
            </div>
          </div>
          ...
          <div style={{ border: '1px solid rgba(255,255,255,0.08)', borderRadius: 24, background: 'linear-gradient(180deg, rgba(255,255,255,0.05), rgba(15,23,32,0.82))', padding: 24, boxShadow: '0 26px 70px rgba(0,0,0,0.25)' }}>
            <div style={{ minHeight: 320, borderRadius: 24, background: 'radial-gradient(circle at top, rgba(156,39,176,0.18), transparent 58%), rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', padding: 22, display: 'grid', gap: 14 }}>
              {['Operations visibility', 'Multi-channel messaging', 'Automation workflow', 'Brand reporting'].map((item) => (
                <div key={item} style={{ display: 'flex', justifyContent: 'space-between', gap: 12, borderTop: '1px solid rgba(255,255,255,0.08)', paddingTop: 12 }}>
                  ...
                  <span style={{ color: '#9C27B0', fontWeight: 800 }}>Live</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <section id="features" style={{ maxWidth: 1200, margin: '0 auto', padding: '72px 20px 24px' }}>
        <div style={{ display: 'grid', gap: 10, marginBottom: 24 }}>
          <p style={{ margin: 0, color: '#9C27B0', fontSize: 12, letterSpacing: '0.18em', textTransform: 'uppercase', fontWeight: 800 }}>Features</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 18 }}>
          {features.map((feature) => (
            <article key={feature.title} style={{ padding: 22, borderRadius: 18, border: '1px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.02)', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.04)' }}>
              <div style={{ width: 42, height: 42, borderRadius: 12, background: 'rgba(156,39,176,0.12)', color: '#9C27B0', display: 'grid', placeItems: 'center', fontWeight: 900 }}>{feature.icon}</div>
      ...
      <section id="bob" style={{ maxWidth: 1200, margin: '0 auto', padding: '52px 20px 24px' }}>
        <div style={{ border: '1px solid rgba(255,255,255,0.08)', borderRadius: 28, background: 'linear-gradient(135deg, rgba(156,39,176,0.12), rgba(13,18,22,0.8))', padding: 28 }}>
          <div style={{ minHeight: 320, borderRadius: 24, background: 'radial-gradient(circle at top, rgba(156,39,176,0.28), transparent 58%), rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', display: 'grid', placeItems: 'center', padding: 20, textAlign: 'center' }}>
            <div style={{ display: 'grid', gap: 14, justifyItems: 'center', width: '100%' }}>
              ...
              <div>
                ...
                <h3 style={{ margin: '10px 0 0', fontSize: 28, lineHeight: 1.1 }}>Premium support for modern fleet conversations</h3>
                <p style={{ margin: '12px auto 0', maxWidth: 320, color: '#ddd6fe', lineHeight: 1.7 }}>Instant answers, clearer guidance, and a more human experience across every delivery journey.</p>
              </div>
              <div style={{ display: 'grid', gap: 10, width: '100%', maxWidth: 620 }}>
                {bobHighlights.map((item) => (
                  <div key={item} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', borderRadius: 14, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', color: '#f5f3ff', fontWeight: 700 }}>
                    <span style={{ width: 10, height: 10, borderRadius: '50%', background: '#9C27B0', boxShadow: '0 0 18px rgba(156,39,176,0.55)' }} />
                    {item}
                  </div>
                ))}
              </div>
              <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 18, marginTop: 8 }}>
                <button type="button" onClick={() => { setBobAnswer(''); setBobLoading(false); setBobOpen(true); }} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', minHeight: 50, padding: '0 22px', borderRadius: 12, background: '#9C27B0', color: '#061018', textDecoration: 'none', fontWeight: 900, border: 'none', cursor: 'pointer' }}>Explore What Bob Can Do</button>
                <a href={cryptoPackageUrl('traderos')} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', minHeight: 50, padding: '0 22px', borderRadius: 12, background: 'rgba(255,255,255,0.05)', color: '#f5f3ff', textDecoration: 'none', fontWeight: 900, border: '1px solid rgba(255,255,255,0.08)' }}>Choose your package</a>
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="consoles" style={{ maxWidth: 1200, margin: '0 auto', padding: '24px 20px 24px' }}>
        <div style={{ border: '1px solid rgba(255,255,255,0.08)', borderRadius: 28, background: 'rgba(255,255,255,0.03)', padding: 28 }}>
          <div style={{ display: 'grid', gap: 10, marginBottom: 18 }}>
            ...
            <h3 style={{ margin: 0, fontSize: 'clamp(28px, 4vw, 42px)' }}>Purpose-built consoles for logistics teams.</h3>
            <p style={{ margin: 0, maxWidth: 760, color: '#ddd6fe', lineHeight: 1.8 }}>Each console supports a distinct part of the fleet workflow, from route review to dispatch support.</p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 16 }}>
            {consoleDetails.map((console) => (
              <article key={console.name} style={{ padding: 20, borderRadius: 18, border: '1px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.02)' }}>
                ...
                <p style={{ margin: '10px 0 0', color: '#ddd6fe', lineHeight: 1.7 }}>{console.description}</p>
                <a href={console.href} style={{ display: 'inline-flex', marginTop: 14, color: '#9C27B0', fontWeight: 800, textDecoration: 'none' }}>Open console →</a>
              </article>
            ))}
          </div>
        </div>
      ...
      <section id="messaging" style={{ maxWidth: 1200, margin: '0 auto', padding: '52px 20px 24px' }}>
        <div style={{ display: 'grid', gap: 10, marginBottom: 24 }}>
          <p style={{ margin: 0, color: '#9C27B0', fontSize: 12, letterSpacing: '0.18em', textTransform: 'uppercase', fontWeight: 800 }}>Messaging Channels</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 14 }}>
          {messagingOptions.map((channel) => (
            <div key={channel} style={{ border: '1px solid rgba(255,255,255,0.08)', borderRadius: 18, padding: 18, background: 'rgba(255,255,255,0.02)' }}>
              <div style={{ width: 54, height: 54, borderRadius: 14, background: 'rgba(156,39,176,0.12)', color: '#9C27B0', display: 'grid', placeItems: 'center', fontWeight: 900 }}>{channel.slice(0, 2).toUpperCase()}</div>
      ...
      <section id="pricing" style={{ maxWidth: 1200, margin: '0 auto', padding: '52px 20px 24px' }}>
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          <p style={{ margin: 0, color: '#9C27B0', fontSize: 12, letterSpacing: '0.18em', textTransform: 'uppercase', fontWeight: 800 }}>Pricing</p>
          <h3 style={{ margin: '16px 0 0', fontSize: 'clamp(32px, 4vw, 46px)' }}>Choose your package</h3>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 18 }}>
          {pricing.map((tier) => (
            <article key={tier.name} style={{ padding: 24, borderRadius: 18, border: tier.featured ? '1px solid ' + '#9C27B0' : '1px solid rgba(255,255,255,0.08)', background: tier.featured ? 'linear-gradient(180deg, rgba(156,39,176,0.12), rgba(10,15,20,0.85))' : 'rgba(255,255,255,0.02)', display: 'grid', gap: 12 }}>
              <div>
                <p style={{ margin: 0, color: '#9C27B0', fontSize: 12, letterSpacing: '0.14em', textTransform: 'uppercase', fontWeight: 800 }}>{tier.name}</p>
              ...
              <a href={cryptoPackageUrl(tier.slug)} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', minHeight: 46, borderRadius: 12, background: '#9C27B0', color: '#071014', fontWeight: 900, textDecoration: 'none' }}>Get started</a>
            </article>
          ))}
        </div>
      </section>
      <section id="signup" style={{ maxWidth: 1200, margin: '0 auto', padding: '8px 20px 24px' }}>
        <div style={{ border: '1px solid rgba(255,255,255,0.08)', borderRadius: 22, background: 'rgba(255,255,255,0.03)', padding: 22, display: 'grid', gap: 14 }}>
          <p style={{ margin: 0, color: '#9C27B0', fontSize: 12, letterSpacing: '0.18em', textTransform: 'uppercase', fontWeight: 800 }}>Signup</p>
      ...
      <section id="contact" style={{ maxWidth: 1200, margin: '0 auto', padding: '24px 20px 12px' }}>
        <div style={{ border: '1px solid rgba(255,255,255,0.08)', borderRadius: 22, background: 'rgba(255,255,255,0.03)', padding: 22, display: 'grid', gap: 14 }}>
          <p style={{ margin: 0, color: '#9C27B0', fontSize: 12, letterSpacing: '0.18em', textTransform: 'uppercase', fontWeight: 800 }}>Contact</p>
      ...
      <footer style={{ borderTop: '1px solid rgba(255,255,255,0.08)', background: '#070c14', marginTop: 60 }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '28px 20px 40px', display: 'flex', flexWrap: 'wrap', gap: 20, justifyContent: 'space-between', alignItems: 'flex-start' }}>
          ...
          <div style={{ display: 'grid', gap: 10, justifyItems: 'start' }}>
            <div style={{ color: '#ddd6fe' }}>hello@FoundLogistics.com</div>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              <a href="/legal" style={{ color: '#9C27B0', textDecoration: 'none', fontWeight: 700 }}>Privacy Policy</a>
              <a href="/legal" style={{ color: '#9C27B0', textDecoration: 'none', fontWeight: 700 }}>Terms</a>
              <a href="/legal" style={{ color: '#9C27B0', textDecoration: 'none', fontWeight: 700 }}>Cookies</a>
            </div>
            <div style={{ color: '#ddd6fe', fontSize: 14 }}>© 2026 FoundLogistics • All rights reserved</div>
          </div>
          <div style={{ width: '100%', marginTop: 8 }}>
            <PremiumSocialLinks accent="#9C27B0" mode="full" label="Social & messaging" />
          </div>
        </div>
      </footer>
      ...
      <button
        ...
        style={{ position: 'fixed', right: 22, bottom: 22, zIndex: 65, width: 90, height: 90, borderRadius: '50%', border: 'none', background: 'radial-gradient(circle at 30% 30%, rgba(255,255,255,0.26), transparent 42%), #9C27B0', color: '#061018', boxShadow: '0 18px 40px rgba(0,0,0,0.3), 0 0 0 1px rgba(255,255,255,0.18), 0 0 28px rgba(156,39,176,0.45)', cursor: 'pointer', animation: 'bobFloat 4s ease-in-out infinite, bobGlow 2.8s ease-in-out infinite' }}
      >
          ...
          <div onClick={() => setBobOpen(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(1, 5, 10, 0.72)', display: 'grid', placeItems: 'center', padding: 20, zIndex: 60 }}>
            <div onClick={(event) => event.stopPropagation()} style={{ position: 'relative', width: 'min(440px, 92vw)', background: 'linear-gradient(180deg, rgba(15, 23, 42, 0.98), rgba(10, 15, 20, 0.98))', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 28, boxShadow: '0 28px 80px rgba(0,0,0,0.45)', padding: 24, color: '#edf7ff' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 14 }}>
                <div style={{ width: 74, height: 74, borderRadius: '50%', background: '#9C27B0', color: '#071014', display: 'grid', placeItems: 'center', fontWeight: 900, fontSize: 28, boxShadow: '0 0 0 8px rgba(255,255,255,0.04)', animation: 'bobFloat 4s ease-in-out infinite' }}>B</div>
                <div>
                  <div style={{ letterSpacing: '0.16em', textTransform: 'uppercase', fontSize: 11, fontWeight: 800, color: '#9C27B0' }}>Bob AI</div>
                  <h3 style={{ margin: '8px 0 0', fontSize: 24 }}>Your smart brand assistant</h3>
                </div>
              </div>
              <p style={{ margin: 0, color: '#bfd8ee', lineHeight: 1.7 }}>Bob AI helps your logistics team track fleet, routes, and deliveries so decisions stay sharp, fast, and well-informed.</p>
              ...
              <div style={{ marginTop: 18, borderRadius: 16, border: '1px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.02)', minHeight: 100, padding: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {bobLoading ? (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, color: '#9C27B0', fontWeight: 700 }}>
                    <span style={{ display: 'inline-flex', gap: 6, alignItems: 'center' }}>
                      <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#9C27B0', animation: 'bobPulse 1s ease-in-out infinite' }} />
                      <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#9C27B0', animation: 'bobPulse 1s ease-in-out 0.15s infinite' }} />
                      <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#9C27B0', animation: 'bobPulse 1s ease-in-out 0.3s infinite' }} />
                    </span>
      ...
      <style jsx>{`
        ...
        @keyframes bobGlow {
          0%, 100% { transform: translateY(0) scale(1); box-shadow: 0 18px 40px rgba(0,0,0,0.3), 0 0 0 1px rgba(255,255,255,0.18), 0 0 28px rgba(156,39,176,0.45); }
          50% { transform: translateY(-2px) scale(1.03); box-shadow: 0 22px 48px rgba(0,0,0,0.32), 0 0 0 1px rgba(255,255,255,0.22), 0 0 34px rgba(156,39,176,0.62); }
        }
