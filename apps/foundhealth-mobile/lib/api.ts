...
// header on every request (see apps/health-console/app/lib/session-auth.ts, which
...
const TOKEN_KEY = 'fo_health_mobile_session_token'
...
export async function fetchOwnBrandMetric(): Promise<BrandMetric | null> {
  ...
  return rows.find((row) => row.brandName.toLowerCase().includes('health')) ?? null
}
