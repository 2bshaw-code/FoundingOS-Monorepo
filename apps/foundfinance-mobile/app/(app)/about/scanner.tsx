...
import { addScannedApproval } from '../../../lib/approvals'
...
export default function ScannerScreen() {
  ...
  const processCapture = async (uri: string) => {
    ...
    try {
      ...
      const detectedAmount = Number(raw?.amountUsd ?? raw?.amount ?? 0) || 0
      setSuggestion({
        title: raw?.name || data?.boltOn || AI_SCANNER.title + ' result',
      ...
      // Honest local flow: there is no real invoicing/expense-submission backend behind
      // FoundFinance's demo mode, so a successfully-scanned receipt is added to the on-device
      // approvals queue (lib/approvals.ts) as a genuinely pending item — it will show up on
      // both the Cash Flow and Approvals tabs, clearly separate from server-provided demo data.
      await addScannedApproval({
        supplier: raw?.merchant || raw?.name || 'Scanned receipt',
        category: raw?.category || 'Uncategorized',
        amountUsd: detectedAmount > 0 ? detectedAmount : 75,
      })
    } catch (err) {
