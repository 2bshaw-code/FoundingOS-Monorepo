...
// header on every request (see apps/finance-console/app/lib/session-auth.ts, which
// checks that header first, falling back to a cookie only for browser-based callers).

// expo-secure-store is native-only (iOS/Android) and throws on web — this app also runs in a
// browser during development/preview, so storage falls back to localStorage there. Native
// builds are unaffected: they always use the real SecureStore APIs below.
const webStorage = {
...
const tokenStore = Platform.OS === 'web' ? webStorage : SecureStore

const AUTH_BASE = 'https://console.foundingos.com'
const TOKEN_KEY = 'fo_finance_mobile_session_token'
...
export async function getToken(): Promise<string | null> {
  return tokenStore.getItemAsync(TOKEN_KEY)
}
...
export async function fetchOwnBrandMetric(): Promise<BrandMetric | null> {
  ...
  return rows.find((row) => row.brandName.toLowerCase().includes('finance')) ?? null
}
