...
export default function InvitationAcceptancePage({ params }: { params: { token: string } }) {
  ...
  return <main className="complete-workspace-access invitation-acceptance">
    <section>
      ...
      {status === 'ready' && details ? <><p className="eyebrow">You have been invited</p><h1>Join {details.tenantName}</h1><p>Set up your FoundingOS account for <strong>{details.email}</strong>. Your role is <strong>{details.role.replaceAll('_', ' ')}</strong>.</p><small className="site-access-note">This invitation expires {new Date(details.expiresAt).toLocaleString()}.</small><form onSubmit={(event) => void accept(event)}><label>Password<input autoComplete="new-password" minLength={12} onChange={(event) => setPassword(event.target.value)} required type="password" value={password} /></label><label>Confirm password<input autoComplete="new-password" minLength={12} onChange={(event) => setConfirmation(event.target.value)} required type="password" value={confirmation} /></label>{message ? <div className="complete-workspace-error" role="alert">{message}</div> : null}<button className="retail-app-primary" type="submit">Accept invitation</button></form></> : null}
    </section>
  </main>
}
