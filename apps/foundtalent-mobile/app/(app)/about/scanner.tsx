...
import { addScannedCandidate } from '../../../lib/pipeline-actions'
...
export default function ScannerScreen() {
  ...
  const [scannedCandidateId, setScannedCandidateId] = useState<string | null>(null)
  ...
  const processCapture = async (uri: string) => {
    ...
    try {
      ...
      const name = raw?.name || data?.boltOn || AI_SCANNER.title + ' result'
      setSuggestion({
        title: name,
        summary: raw?.whatsappMessage || `Detected a photo capture and mapped it into ${BRAND.name}'s workflow.`,
        confidenceScore: raw?.confidenceScore ?? 0.95,
        raw,
      })

      // Feed the scanned CV into the Pipeline tab as a real, on-device candidate entry — this
      // app has no real ATS write-backend to persist a new candidate to, so it's stored locally
      // (lib/pipeline-actions.ts) and clearly labelled "scanned from CV" wherever it's shown,
      // rather than silently discarding the scan result once this screen closes.
      const candidateId = `scan-${Date.now()}`
      await addScannedCandidate({
        id: candidateId,
        name: typeof name === 'string' && name !== AI_SCANNER.title + ' result' ? name : 'Scanned candidate',
        headline: `CV scan — ${AI_SCANNER.title}`,
        jobId: 'job-unassigned',
        jobTitle: 'Unassigned role',
        stage: 'applied',
        yearsExperience: 0,
        skills: [],
        appliedDaysAgo: 0,
        matchScorePct: Math.round((raw?.confidenceScore ?? 0.95) * 100),
      })
      setScannedCandidateId(candidateId)
    } catch (err) {
        ...
        <View style={[styles.resultCard, { borderColor: BRAND.accent }]}>
          ...
          {scannedCandidateId ? (
            <Pressable
              ...
              onPress={() => router.push(`/candidate/${scannedCandidateId}`)}
            >
              <Text style={styles.actionButtonText}>View in Pipeline</Text>
            </Pressable>
          ) : null}
          <Pressable
            style={[styles.retryButton, { borderColor: BRAND.accent }]}
            onPress={() => { setSuggestion(null); setImageUri(null); setScannedCandidateId(null) }}
          >
