...
export async function middleware(request: NextRequest) {
  ...
  // Self-contained synthetic data generators (this brand's own real cron-backed endpoints)
  // must be reachable with NO session at all — that's exactly how Vercel's own cron jobs
  // invoke them on schedule (crons carry no browser cookie). This app was missing this
  // exemption entirely, meaning its real Vercel cron(s) have been silently redirected to
  // the login page instead of actually running since they were set up. Each one only ever
  // mutates this brand's own BrandMetric/synthetic data via a deterministic, seeded,
  // no-external-network generator — no real exposure or write risk.
  const SYNTHETIC_GENERATOR_PATHS = new Set(['/api/scrape/refresh', '/api/dashboard/refresh'])
