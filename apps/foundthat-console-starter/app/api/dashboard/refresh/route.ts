...
// Full Demo Mode: read-only, deterministic combined dashboard metrics for FoundThat. No DB
// writes, no external network calls, no secrets — safe to call from a Vercel cron every
// 5 minutes.
export async function GET() {
  const windowMs = 5 * 60 * 1000
  const activeUsers = Math.floor(seededValue('foundthat:activeUsers', windowMs, 40, 0.5))
  const ordersToday = Math.floor(seededValue('foundthat:ordersToday', windowMs, 120, 0.4))
  const revenueTodayUsd = Number(seededValue('foundthat:revenueToday', windowMs, 4800, 0.35).toFixed(2))
  const openAlerts = Math.floor(hashSeed(`foundthat:alerts:${timeBucket(windowMs)}`) * 5)
