...
import { authClient, submitPendingApplication } from '@core_workforce/auth'
...
export function LoginPage() {
  ...
  return <ErrorBoundary fallback={<Fallback />}><AuthLayout brandName="CoreWorkforce" logo={<CoreWorkforceBrandMark className="h-12 w-12" />}><PageContainer><LoginForm title="CoreWorkforce account sign in" onLogin={async (email, password) => finish(await authClient.login(email, password))} onPasskeyLogin={async (email) => finish(await authClient.loginWithPasskey(email))} /></PageContainer></AuthLayout></ErrorBoundary>
}
