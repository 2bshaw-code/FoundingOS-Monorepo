...
export function CoreWorkforceConsole({ title }: { title: string }) {
  const [dashboard, setDashboard] = useState<Dashboard | null>(null)
  const [workflowDemo, setWorkflowDemo] = useState<WorkflowDemo | null>(null)
  const [notice, setNotice] = useState('')
  useEffect(() => {
    authClient.request<{ success: true; data: Dashboard }>('/dashboard').then((response: { data: Dashboard }) => setDashboard(response.data)).catch((error: unknown) => setNotice(error instanceof Error ? error.message : 'CoreWorkforce data unavailable'))
  }, [])
