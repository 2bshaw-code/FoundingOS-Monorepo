...
// is stored centrally by core-operations (POST /events/publish). Talent
...
// block the actual Talent workflow (timesheet approval, payroll run).
...
export const publishFeedEvent = (type: string, payload: Record<string, unknown>) => {
  fetch(EVENT_FEED_URL, {
    ...
    body: JSON.stringify({ type, source: 'talent', payload }),
  }).catch(() => {})
}
