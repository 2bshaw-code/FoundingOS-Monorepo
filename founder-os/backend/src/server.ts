...
import { authRouter, talentAuthRouter } from './auth.js'
...
app.get('/health', (_req, res) => res.json({ app: 'founder-os', status: 'ok' }))
app.use('/api/v1/auth', createRateLimit({ windowMs: 15 * 60_000, max: 20 }))
app.use('/api/v1/founder/auth', createRateLimit({ windowMs: 15 * 60_000, max: 20 }))
app.use('/api/v1/talent/auth', createRateLimit({ windowMs: 15 * 60_000, max: 20 }))
app.use('/api/v1', createRateLimit({ max: 240 }))
app.use('/api/v1/auth', authRouter)
app.use('/api/v1/founder/auth', authRouter)
app.use('/api/v1/talent/auth', talentAuthRouter)
app.use('/api/v1/passkeys', passkeyRouter)
app.use('/api/v1', apiRouter)
app.use(structuredErrorHandler)
app.listen(port, () => console.log(`FoundingOS API listening on ${port}`))
