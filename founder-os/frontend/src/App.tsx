...
function GlobalFounderAI() {
  ...
  const [messages, setMessages] = useState([{ from: 'foundai', text: 'IntelligenceAI is ready to help with FoundingOS onboarding, the console, and company control.' }])
  ...
  const submit = (event: FormEvent) => {
    ...
    const reply = /console|dashboard/.test(text.toLowerCase())
      ? 'IntelligenceAI can open the console, explain the current page, and help you move through FoundingOS faster.'
      : /onboard|start|join|apply/.test(text.toLowerCase())
        ? 'IntelligenceAI is the onboarding layer for FoundingOS and can guide users from first visit to a working console.'
        : 'IntelligenceAI helps with FoundingOS navigation, company setup, and the next operational step.'
  ...
  return (
    <>
      <button type="button" onClick={() => setOpen(true)} className="bob-ai-circle fixed bottom-5 right-5 z-40 grid h-14 w-14 place-items-center rounded-full bg-[#006CFF] text-xl font-bold text-white shadow-xl" aria-label="Open IntelligenceAI">AI</button>
      {open && (
        <section className="fixed inset-x-4 bottom-4 z-50 ml-auto max-w-sm overflow-hidden rounded-lg border border-[#8BBEFF] bg-white shadow-2xl sm:right-6">
          <header className="flex items-center justify-between bg-[#006CFF] px-4 py-3 text-white">
            <strong>IntelligenceAI · {context}</strong>
            <button type="button" onClick={() => setOpen(false)} className="text-2xl" aria-label="Close IntelligenceAI">×</button>
          </header>
          ...
          <form onSubmit={submit} className="flex gap-2 border-t border-[#D9E3F0] p-3">
            <input value={prompt} onChange={(event) => setPrompt(event.target.value)} className="min-w-0 flex-1 rounded border border-[#A8BDD7] px-3 py-2 text-sm" placeholder="Ask IntelligenceAI about FoundingOS" />
