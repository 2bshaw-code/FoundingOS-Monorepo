...
import { Card, Header } from "@founder-os/ui";
...
// CoreOperations and CoreOperations are deprecated brands (removed) — see /docs/deprecations.md
const modules = ["core_operations", "foundthis", "core_intelligence", "core_workforce"] as const;
const moduleLabel = (module: (typeof modules)[number]) =>
  module === "core_operations"
    ? "CoreOperations"
    : module === "foundthis" || module === "core_intelligence"
      ? "Intelligence"
      : "CoreWorkforce";
          ...
          <Card key={company.id} title={company.name}>
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                {company.slug === "core_operations" ? (
                  <CoreOperationsBrandMark className="h-9 w-9" />
                ) : company.slug === "foundthis" || company.slug === "core_intelligence" ? (
                  <IntelligenceBrandMark className="h-9 w-9" />
                ) : company.slug === "core_workforce" ? (
                  <CoreWorkforceBrandMark className="h-9 w-9" />
                ) : (
                  <span
            ...
            <div className="mt-4 grid gap-3 border-t border-[var(--line)] pt-4 sm:grid-cols-3">
              <label className="grid gap-1 text-xs font-semibold">
                ...
                <input
                  type="color"
                  value={company.slug === "foundthis" || company.slug === "core_intelligence" ? "#FFD600" : company.slug === "core_workforce" ? "#F97316" : company.slug === "core_operations" ? "#25D366" : company.settings?.brandColor || "#006CFF"}
                  disabled={company.slug === "foundthis" || company.slug === "core_intelligence" || company.slug === "core_workforce" || company.slug === "core_operations"}
                  onChange={(event) =>
                    void update(company, {
                      settings: {
                        ...company.settings,
