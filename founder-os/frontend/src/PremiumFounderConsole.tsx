...
import { AnalyticsMetricCard, BobMediaStudio, Card, DeliverySuite, EntityAnalytics, FunnelChart, FounderOsLogo, ImageBlock, InsightPanel, InventoryModule, InvoicesModule, MarketingSuite, OperationsAnalytics, OrdersModule, PasskeySettings, PipelineAnalytics, PremiumConsole, SocialScheduler, Table, WaterfallChart, WeatherLocationSuite, WeatherLocationWidget, type OperationsClient } from '@founder-os/ui'
...
const tabs = [
  ...
  { id: 'marketing', label: 'Marketing Suite' }, { id: 'social', label: 'Social Scheduler' }, { id: 'media', label: 'IntelligenceAI Media Studio' }, { id: 'location', label: 'Weather & Location' },
...
export function PremiumFounderConsole() {
  ...
  const [services, setServices] = useState({ core_operations: false, core_workforce: false })
  ...
  const load = async () => {
    try {
      const [ecosystem, companyResponse] = await Promise.all([
        authClient.request<{ success: true; data: { core_operations: { available: boolean; data?: Pipeline }; core_workforce: { available: boolean } } }>('/insights/ecosystem'),
        authClient.request<{ success: true; data: Company[] }>('/companies'),
      ])
      if (ecosystem.data.core_operations.data) setPipeline(ecosystem.data.core_operations.data)
      setServices({ core_operations: ecosystem.data.core_operations.available, core_workforce: ecosystem.data.core_workforce.available })
  ...
  return <PremiumConsole brand="FoundingOS" eyebrow="Master control centre" title="FoundingOS" description="Commercial intelligence, company control, and ecosystem operations." tabs={tabs} activeTab={tab} onTabChange={setTab} accent="#006CFF" logo={<FounderOsLogo className="h-11 w-11"/>}>
    ...
    {tab === 'intelligence' && <div className="space-y-5"><Card title="Core.Intelligence"><p className="text-sm text-[var(--muted)]">First-party KPI, funnel, and reporting data from FoundingOS suites.</p></Card><FunnelChart data={[{label:'Operational leads',value:pipeline.leads.length},{label:'Customers',value:pipeline.customers.length},{label:'Orders',value:pipeline.orders.length}]}/></div>}
    {tab === 'operations' && <><div className="grid gap-4 md:grid-cols-4"><InsightPanel title="CoreOperations" value={services.core_operations ? 'Connected' : 'Unavailable'} tone={services.core_operations ? 'positive' : 'warning'}/><InsightPanel title="Intelligence" value="Connected" tone="positive"/><InsightPanel title="CoreWorkforce" value={services.core_workforce ? 'Connected' : 'Unavailable'} tone={services.core_workforce ? 'positive' : 'warning'}/><InsightPanel title="Messages" value={String(pipeline.metrics.messages)} detail="FoundingOS communication events"/></div><Card title="Multi-platform operation routing"><div className="grid gap-3 md:grid-cols-2">{multiPlatformMessaging.map((item) => <div key={`${item.platform}-ops`} className="rounded-xl border border-[var(--line)] bg-[#F2F7FF] p-3"><div className="flex items-center justify-between"><span className="text-sm font-semibold text-[var(--ink)]">{item.platform}</span><span className="rounded-full border border-[#DCE3EC] bg-white px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--muted)]">{item.status}</span></div><p className="mt-2 text-sm leading-6 text-[var(--muted)]">{item.detail}</p></div>)}</div></Card></>}
