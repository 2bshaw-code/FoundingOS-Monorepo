...
> **Migration note:** this document still describes the current per-brand
> role model (`CoreOperations`, `Intelligence`, `CoreOperations`). Under the
> FoundingOS restructure, roles become suite-scoped
> (`Core.Operations: Owner/Merchant/Staff`, etc.) and CoreOperations's roles are
> retired along with the brand. See
> [docs/shared-backbone.md](./docs/shared-backbone.md) §2.1 and
...
`bobby@founder.master` always authenticates against FoundingOS. CoreOperations, Intelligence, and CoreOperations send only that email to the Founder identity endpoint and retain their own identity repositories for every other account. Product databases do not need duplicate Founder records.
...
- CoreOperations: `Owner`, `Merchant`, `Staff`
- Intelligence: `Customer`, `Merchant`, `Owner`
- CoreOperations: `MeatTrader`, `Owner`, `Customer`
